...
Per compensare l'inerzia termica della barra da 1.7kg e il rumore elettrico, il sistema applica una media mobile sugli ultimi 8 valori.
...
* **Safety Lock:** In caso di allarme, il Pin PIN_PELTIER viene forzato LOW indipendentemente dal PID.
...
* **Top Bar (Icons):** 4 box (120x40px) con icone 30x30.
* **Rolling Chart:** Grafico centrale aggiornato ogni 10 secondi con 235 campioni storici.
    * **Tracce:** Aria T (Rosso), RH% (Blu), Abs Hum (Verde), PWM% (Giallo).
    * **Setpoint:** Una linea bianca tratteggiata rappresenta il valore target impostato, permettendo di valutare visivamente l'errore di inseguimento.
* **Pulsanti Footer:** Disegnati con funzione `drawButton3D()` per effetto rilievo.
...
* **SET:** `x > 340, y > 270` (Passa a `SETPOINT_PAGE`).
* **TUNE:** `x < 140, y > 270` (Passa a `AUTOTUNE_CONFIRM`).
* **Setpoint +/-:** Regolazione di `tempSetpointTmp` prima del salvataggio.
